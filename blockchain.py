...
class Blockchain:
    ...
    def load_data(self):
        """Initialize self.chain + open transactions data from a file."""

        try:
            with open('self.chain.txt', mode='r') as f:
                ...
                for block in self.chain:
                    ...
                    updated_self_chain.append(updated_block)
                self.chain = updated_self_chain
            ...
            # Our starting block for the self.chain
            genesis_block = Block(0, '', [], 100, 0)
            # Initializing our (empty) self.chain list
    ...
    def save_data(self):
        """Save self.chain + open transactions snapshot to a file."""
        try:
            with open('self.chain.txt', mode='w') as f:
    ...
    def get_balance(self):
        ...
        # This fetches sent amounts of transactions that were already included in blocks of the self.chain
        ...
        # This fetches received coin amounts of transactions that were already included in blocks of the self.chain
    ...
    def get_last_blockchain(self):
        """ Returns the last value of the current self.chain. """
        if len(self.chain) < 1:
            return None
        return self.chain[-1]
    ...
    def add_transaction(self,recipient, sender, amount=1.0):
        """ Append a new value as well as the last self.chain value to the self.chain.
    ...
    def mine_block(self):
        ...
        # Fetch the currently last block of the self.chain
        last_block = self.chain[-1]
